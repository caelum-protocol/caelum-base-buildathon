...
export default function WhisperBox() {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4">
      <div className="mb-2 text-sm opacity-80">Signed in as: <b>{user}</b></div>
      <div className="space-y-3 mb-4">
        {messages.map(m => (
          <div key={m.id} className={m.role === "user" ? "text-right" : "text-left"}>
            <div className={`inline-block rounded-2xl px-4 py-2 shadow ${m.role === "user" ? "bg-gray-200" : "bg-gray-100"}`}>
              <div>{m.text}</div>
              {m.role === "caelum" && (
                <div className="mt-2 flex gap-2 text-xs opacity-80">
                  <button className="underline" onClick={() => queueForMint(m.refId)}>⭐ Queue for Mint</button>
                  <button className="underline" onClick={() => sendFeedback(m.refId, 1)}>👍</button>
                  <button className="underline" onClick={() => sendFeedback(m.refId, -1)}>👎</button>
                </div>
              )}
            </div>
        ...
        {typing && (
          <div className="text-left">
            <div className="inline-block rounded-2xl px-4 py-2 shadow bg-gray-100 animate-pulse">Caelum is thinking…</div>
          </div>
        )}
      </div>
      ...
      <form onSubmit={sendWhisper} className="flex gap-2">
        ...
        />
        <button className="px-4 py-2 rounded-xl border shadow" type="submit">Send</button>
      </form>
    </div>
  );
}